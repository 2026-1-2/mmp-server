...
export class LogsController {
  ...
  @ApiResponse({ status: 200, description: '페이지네이션된 활동 로그 목록' })
  listActivity(
  ...
  @ApiQuery({ name: 'from', required: false, type: String, example: '2026-01-01T00:00:00Z' })
  @ApiQuery({ name: 'to', required: false, type: String, example: '2026-12-31T23:59:59Z' })
  @ApiResponse({ status: 200, description: '시스템 로그 목록' })
  listSystem(
  ...
  @ApiBody({
    schema: {
      example: { log_level: 'ERROR', source: 'MEDIAMTX', message: '연결 끊김', detail: '...' },
    },
  })
  @ApiResponse({ status: 201, description: '로그 저장 완료' })
  createSystemLog(
